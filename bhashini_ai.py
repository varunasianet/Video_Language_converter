...
def translate_text_for_review(input_text, input_language, target_languages):
    input_language_code = input_language.split('(')[1].split(')')[0].split('-')[0]
    language_codes = [lang.split(' ')[1][1:-1].split('-')[0] for lang in target_languages]
    
    translation_results = translate_text(input_text, language_codes, input_language_code)
    
    outputs = []
    for lang in ['ta', 'en', 'hi', 'kn', 'ml', 'bn', 'te']:
        translation_result = next((r for r in translation_results if r['to'] == lang), None)
        if translation_result:
            outputs.append(translation_result.get('text', 'Translation not available'))
        else:
            outputs.append('')
    
    return outputs

def process_translation_and_tts(tamil_text, english_text, hindi_text, kannada_text, malayalam_text, bengali_text, telugu_text, input_language, file_type, gender, original_file):
    input_language_code = input_language.split('(')[1].split(')')[0].split('-')[0]
    
    outputs = []
    all_languages = ['ta', 'en', 'hi', 'kn', 'ml', 'bn', 'te']
    translations = [tamil_text, english_text, hindi_text, kannada_text, malayalam_text, bengali_text, telugu_text]
    
    for lang, translated_text in zip(all_languages, translations):
        if translated_text and translated_text.strip():  # Check if the translation is not empty
            ...
            outputs.append(None)  # Add placeholder if no translation
...
with gr.Blocks() as iface:
    ...
    for lang in ['Tamil', 'English', 'Hindi', 'Kannada', 'Malayalam', 'Bengali', 'Telugu']:
        translation_boxes.append(gr.Textbox(label=f"Translated Text ({lang}) - Edit if needed"))

    with gr.Row():
        gender_input = gr.Radio(choices=['male', 'female'], label="Select Gender")

    process_button = gr.Button("Process TTS and Video")

    output_boxes = []
    for lang in ['Tamil', 'English', 'Hindi', 'Kannada', 'Malayalam', 'Bengali', 'Telugu']:
        output_boxes.append(gr.Video(label=f"Generated Video ({lang})"))
    ...
    translate_button.click(
        translate_text_for_review,
        inputs=[transcribed_text, input_language_input, language_input],
        outputs=translation_boxes
    )
